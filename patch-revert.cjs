...
let llmCode = `import { extractContent } from '../utils/parserUtils';
...
`;
fs.writeFileSync('src/services/llmService.js', llmCode);

let transformCode = fs.readFileSync('src/components/transform/TransformPage.jsx', 'utf8');
transformCode = transformCode.replace(/await new Promise\(r => setTimeout\(r, 100\)\);/g, 'await new Promise(r => setTimeout(r, 6000));');
fs.writeFileSync('src/components/transform/TransformPage.jsx', transformCode);

console.log('Reverted exactly to working state!');
